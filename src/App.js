...
const App = () => {
  ...
  const configureCampaign = async () => {
    try {
      ...
      const { data: campaignStats } = await client.get(`/tiltify/${userId}/${campaignSlug}`);
      if (campaignStats.isCached) {
        // We've cached these donations and stats before, redirect now.
      }

      setCampaign(campaignStats.campaign);

      setLoadingStatus('Getting Donations from Tiltify');
      // Cache Donations
      // const cacheDonationsResult = await client.get(`/tiltify/${userId}/${campaignSlug}/cache`);
      // check cacheDonationsResult status
      await sleep(5000);

      setLoadingStatus('Calculating Stats');
      // Calculate Stats
      // const calculateStatsResult = await client.get(`/tiltify/${userId}/${campaignSlug}/calculate`);
      // check cacheDonationsResult status
      await sleep(5000);

      // window.location = /userId/campaignSlug
    } catch(e) {
      console.log(e);
    } finally {
      setLoading(false);
